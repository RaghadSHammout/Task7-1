...
export default function PricingCard({variant}) {
  return (
    <div className={`PricingCard border rounded p-3 ${variant === 'second' ? 'second-style' : ''}`}>
      <div className="card-body">
        <div className="card-top d-flex justify-content-between text-white p-2 rounded mb-3 ">
          <div>
            <h5>Regular Plan</h5>
            <h3>$49<span className="fs-5">/Month</span></h3>
          </div>
        ...
        <button className="btn card-plan-btn py-2 w-100 mt-3 border">GET STARTED NOW →</button>
      </div>
    </div>
  );
}
